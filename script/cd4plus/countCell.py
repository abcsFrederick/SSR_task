...
for indexO in range(NumOfMasks):
    print("Processing Mask" + str(indexO))
    maskPath = eval("Mask" + str(indexO))
    try:
    ...
    for indexI, include in enumerate(include_union_polygons):
        ...
        print(end - start)
        # img = Image.new('L', (tileInArray.shape[1], tileInArray.shape[0]), 0)
        # ImageDraw.Draw(img).polygon(include_polygon, outline=True, fill=True)
        # mask = np.array(img, dtype=bool)
        # mask = np.invert(mask)
        # include_masked_img = np.ma.array(im, mask=mask, fill_value=0)
        # include_masked_img = include_masked_img.filled()
        # include_masked_img = Image.fromarray(include_masked_img)


        differenceROIs = Polygon(include_polygon)
        if exclude_flag:
            # if exclude_union_polygons.type == 'MultiPolygon':
            ...
            # elif exclude_union_polygons.type == 'Polygon':
            #     exclude_polygon_x, exclude_polygon_y = exclude_union_polygons.exterior.xy
            #     exclude_polygon_left = np.amin(exclude_polygon_x, axis=0)
            #     exclude_polygon_right = np.amax(exclude_polygon_x, axis=0)
            #     exclude_polygon_top = np.amin(exclude_polygon_y, axis=0)
            #     exclude_polygon_bottom = np.amax(exclude_polygon_y, axis=0)
        ...
        for indexD, differenceROI in enumerate(differenceROIs):
            ...
            # print("numpy array Mask to PIL image....")
            # start = time.time()
            # maskarr = Image.fromarray(mask)
            # end = time.time()
            # print(end - start)
