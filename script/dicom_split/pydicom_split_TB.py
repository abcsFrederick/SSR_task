...
#                     help='order of patient placed in scanner', default='1,1,1')
# parser.add_argument('-orderT', '--orderT',
#                     help='order of patient placed in scanner of top bed', default='1,1,1')
# parser.add_argument('-orderB', '--orderB',
#                     help='order of patient placed in scanner of bottom bed', default='1,1,1')
...
tmpDir = os.path.dirname(topFolder0)  # noqa
Outdir = outPath  # noqa
# topFolder = topFolder
subfolders = subfolders  # noqa
axis = axis  # noqa
order = order  # noqa
orderT = orderT  # noqa
orderB = orderB  # noqa
offset = offset  # noqa
n_of_split = n_of_split  # noqa
# shared = not kwargs.pop('unique_study_instance_uids')
# if shared and not kwargs.get('study_instance_uids'):
#     n = len(kwargs.get('series_instance_uids')) or kwargs.get('n')
#     kwargs['study_instance_uids'] = [x667_uuid() for i in range(n)]
...
for index in range(len(subfolders)):
    ...
    if not orderT[index]:
        ...
        kwargs = {
            ...
            "order": order[index], "output_dir": Outdir,
            "study_instance_uids": [x667_uuid() for i in range(n)]
        }
    else:
        ...
        kwargs = {
            ...
            "offset": int(offset[index]), "output_dir": Outdir,
            "study_instance_uids": [x667_uuid() for i in range(n)]
        }
    # print kwargs
    split_dicom_directory(directory, **kwargs)
