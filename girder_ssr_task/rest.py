...
from ssr_tasks.test.test import testfunc
...
class SSR_task(Resource):
                    ...
                    for study in studyFolders:
                        ...
                        for itemObj in seriesItems:
                            ...
                            try:
                                ...
                                item['patient_folder_id'] = patient["_id"]
    ...
    def dicom_split(self, inputIds, inputType, subfolders, n, axis, order, orderT, orderB, offset, pushFolderId, pushFolderName):
        ...
        if inputType == "archive":
            # get full path by id
            study_description, inputFolders = ArchiveFolder().fullPath(inputIds, "study")
            if not os.path.isdir(inputFolders):
                raise ValidationException("path %s is not exist" % inputFolders)
        elif inputType == "girder":
            inputFolders = []
            for eachId in inputIds:
                inputFolders.append(Folder().load(eachId, level=AccessType.READ, user=self.user))

        pushFolder = Folder().load(pushFolderId, level=AccessType.READ, user=self.user)
        return DicomSplit().createJob(inputFolders, inputIds, inputType, subfolders,
    ...
    @access.public
    @autoDescribeRoute(
        Description("Inference image with rnascope or cd4+ module.")
    ...
    def test(self):
        return testfunc.delay(girder_job_title='girder_job_title', girder_job_type='girder_job_type')
    ...
    @access.user
    @autoDescribeRoute(
        Description("Get annotations from Halo link.")
        .jsonParam("itemIds", "item ids of WSIs.", required=True)
        .param("username", "username of aperio portal.", required=True)
        .param("password", "password of aperio portal.", required=True)
    )
