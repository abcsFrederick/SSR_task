...
class RNAScope(AccessControlledModel):
    ...
    def createJob(self, fetchWSIs, fetchCSVFiles, csvFileIds, user, token, itemIds,
                  includeAnnotationIds, roundnessThresholds, pixelThresholds,
        ...
        if slurm is True:
            ...
            job = Job().createJob(title=title, type='rnascope', handler='worker_handler', user=user)

        # outPath = os.path.join(Setting().get(PluginSettings.GIRDER_WORKER_TMP), 'pushFolderName')
        print(Setting().get(PluginSettings.GIRDER_WORKER_TMP))
