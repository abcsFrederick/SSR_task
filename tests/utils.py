...
class Utils(object):
    ...
    def uploadFileToItem(self, hashPath, parentFolder, folderName, user, fsAssetstore):
        folder = Folder().createFolder(
            parent=parentFolder, name=folderName, parentType='folder', reuseExisting=True, creator=user)

        file = self.uploadExternalFile(
            hashPath, user, fsAssetstore, folder['name'], os.path.basename(hashPath).replace('.sha512', ''))
