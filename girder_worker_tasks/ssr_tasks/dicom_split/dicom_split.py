...
@app.task(bind=True)
def dicom_split(self, folderPaths, **kwargs):
    # Flatten absolute path
    paths = [path for folderPath in folderPaths for path in folderPath]
    print(paths)
    outputPath = kwargs.get('outputPath')
    axis = kwargs.get('axis')
    order = kwargs.get('order')
    orderT = kwargs.get('orderT')
    orderB = kwargs.get('orderB')
    offset = kwargs.get('offset')
    n_of_split = kwargs.get('n_of_split')
    print('output path: ' + outputPath)
    start_processing(outputPath, paths, axis, order, orderT, orderB,
                     offset, n_of_split)
    return outputPath
...
class DICOMSplitterTB:
    ...
    def __iter__(self):
        ...
        if self._pixel_array is not None:
            ...
            self._offsetInPx = math.floor(self._offset * 1.0 / 100 * sizeR)  # python2
    ...
    for root, subdirs, files in os.walk(directory):
        if len(files):
            if files.pop(0) != '.DS_Store':
                ...
                newRoot_ctpt = os.path.join(output_dir, root.split('/')[-4], root.split('/')[-1])
                # newRoot = output_dir
                # for subdirs in root.split('/')[1:]:
                #     newRoot = os.path.join(newRoot, subdirs)
                if not os.path.exists(newRoot):
                    os.makedirs(newRoot)
                yield root, newRoot, newRoot_ctpt
...
def split_dicom_directory(directory, axis=0, n=3, nTB=None, offset=5, keep_origin=False,
                          ...
                          mangle_output_paths=False, order='1,1,1', orderT=None, orderB=None):
    if nTB is not None:
        orderT = orderT.split(',')
        orderB = orderB.split(',')
        if int(nTB[0].split(',')[0]) != len(orderT):
            raise Exception('[ERROR] # of split has to equal to length of order on Top')
        if int(nTB[0].split(',')[1]) != len(orderB):
        ...
        if study_instance_uids and len(study_instance_uids) != n:
            raise ValueError
    for directoryChecked, newRoot, newRoot_ctpt in checkDirectory(directory, output_dir):
        for path, dataset in DICOMDirectory(directoryChecked):
            try:
                ...
                pixel_array = None
            if nTB is not None:
                nT = int(nTB[0].split(',')[0])
                nB = int(nTB[0].split(',')[1])
            ...
            if nTB is not None:
                ...
                parsed, dataset.SourcePatientGroupIdentificationSequence, trailing = get_patient(dataset.PatientName,
                                                                                                 dataset.PatientID, n,
                                                                                                 patient_names,
                                                                                                 patient_ids, order)
            ...
            for i, origin, pixel_array in dicom_splitter:
                if parsed_patient_names[i] != 'blank':
                    ...
                    if output_paths:
                        ...
                        output_path = None
                    if split_dataset.Modality == 'PT' or split_dataset.Modality == 'CT':
                        created_output_path = make_output_path(newRoot_ctpt,
                                                               parsed_patient_names[i], output_path)
                    else:
                        created_output_path = make_output_path(newRoot, parsed_patient_names[i],
                                                               output_path)
...
def start_processing(outputPath, folderPaths, axis, order, orderT, orderB,
    ...
    for index, folderPath in enumerate(folderPaths):
        ...
        if not orderT[index]:
            ...
            kwargs = {
                'axis': int(axis[index]), 'n': int(n_of_split[index]),
                'order': order[index], 'output_dir': outputPath,
                'study_instance_uids': [x667_uuid() for i in range(n)]
            }
        else:
            ...
            kwargs = {
                'axis': int(axis[index]), 'nTB': nTB,
                'orderT': orderT[index], 'orderB': orderB[index],
                'offset': int(offset[index]), 'output_dir': outputPath,
                'study_instance_uids': [x667_uuid() for i in range(n)]
            }
        # print kwargs
        split_dicom_directory(folderPath, **kwargs)
